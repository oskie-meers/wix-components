...
class MhwwFloorPlan extends HTMLElement {
  connectedCallback() {
    ...
    style.textContent = FLOOR_CSS;
    shadow.appendChild(style);

    // Viewport wrapper
    const vpOuter = document.createElement('div');
    vpOuter.className = 'vp-outer';

    const vpInner = document.createElement('div');
    vpInner.className = 'vp-inner';
    vpInner.innerHTML = FLOOR_HTML;
    vpOuter.appendChild(vpInner);

    // Zoom controls
    const controls = document.createElement('div');
    controls.className = 'fp-controls';
    controls.innerHTML = `
      <button class="fp-btn" id="fp-zoom-in" title="Zoom in">+</button>
      <button class="fp-btn" id="fp-zoom-out" title="Zoom out">&minus;</button>
      <button class="fp-btn" id="fp-zoom-fit" title="Fit to screen" style="font-size:13px;">⤢</button>
    `;
    vpOuter.appendChild(controls);

    // Minimap
    const minimap = document.createElement('div');
    minimap.className = 'fp-minimap';
    const minimapFloor = document.createElement('div');
    minimapFloor.className = 'fp-minimap-floor';
    // Clone simplified visual — just coloured rects for zones (lightweight)
    minimapFloor.innerHTML = `
    ...
    `;
    const minimapVp = document.createElement('div');
    minimapVp.className = 'fp-minimap-vp';
    minimapFloor.appendChild(minimapVp);
    minimap.appendChild(minimapFloor);
    vpOuter.appendChild(minimap);

    // Hint
    const hint = document.createElement('div');
    hint.className = 'fp-hint';
    hint.textContent = 'Pinch to zoom · Drag to pan';
    vpOuter.appendChild(hint);

    shadow.appendChild(vpOuter);

    // ── Pan / Zoom State ──────────────────────────────────────────────
    const CONTENT_W = 1300, CONTENT_H = 540;
    const MIN_ZOOM = 0.3, MAX_ZOOM = 2.5;
    let zoom = 1, panX = 0, panY = 0;
    let isDragging = false, dragStartX = 0, dragStartY = 0, panStartX = 0, panStartY = 0;

    function clampPan(x, y, z) {
      const vw = vpOuter.clientWidth  || window.innerWidth;
      const vh = vpOuter.clientHeight || 540;
      const maxX = 0;
      const minX = Math.min(0, vw  - CONTENT_W * z);
      const maxY = 0;
      const minY = Math.min(0, vh - CONTENT_H * z);
    ...
    function fitToScreen() {
      const vw = vpOuter.clientWidth  || window.innerWidth;
      const vh = vpOuter.clientHeight || 540;
      zoom = Math.min(vw / CONTENT_W, vh / CONTENT_H, 1);
      const cx = (vw  - CONTENT_W * zoom) / 2;
      const cy = (vh - CONTENT_H * zoom) / 2;
      [panX, panY] = [cx, cy];
      apply();
    }
    ...
    function updateMinimap() {
      ...
      const vh = vpOuter.clientHeight || 540;
      const mw = 160, mh = 67;
      const scaleX = mw / CONTENT_W, scaleY = mh / CONTENT_H;
      // Viewport rect in content space
      const vpLeft   = -panX / zoom;
      const vpTop    = -panY / zoom;
      const vpWidth  = vw  / zoom;
      const vpHeight = vh / zoom;
      minimapVp.style.left   = Math.max(0, vpLeft   * scaleX) + 'px';
      minimapVp.style.top    = Math.max(0, vpTop    * scaleY) + 'px';
      minimapVp.style.width  = Math.min(mw, vpWidth  * scaleX) + 'px';
      minimapVp.style.height = Math.min(mh, vpHeight * scaleY) + 'px';
    }
    ...
    vpOuter.addEventListener('wheel', e => {
      e.preventDefault();
      const rect = vpOuter.getBoundingClientRect();
    ...
    // ── Mouse drag pan ────────────────────────────────────────────────
    vpOuter.addEventListener('mousedown', e => {
    ...
    let lastTouches = null, lastPinchDist = null, touchPanX = 0, touchPanY = 0;
    ...
    vpOuter.addEventListener('touchstart', e => {
      e.preventDefault();
    ...
    vpOuter.addEventListener('touchmove', e => {
      e.preventDefault();
      const rect = vpOuter.getBoundingClientRect();
      if (e.touches.length === 2) {
        ...
        if (lastPinchDist) {
          zoomAt(zoom * (dist / lastPinchDist), mid.x, mid.y);
        }
        lastPinchDist = dist;
      } else if (e.touches.length === 1) {
    ...
    vpOuter.addEventListener('touchend', e => {
      lastPinchDist = null;
    ...
    // ── Button controls ───────────────────────────────────────────────
    shadow.getElementById('fp-zoom-in').addEventListener('click', e => {
      e.stopPropagation();
      const vw = vpOuter.clientWidth / 2;
      const vh = (vpOuter.clientHeight || 540) / 2;
      zoomAt(zoom * 1.3, vw, vh);
      vpInner.style.transition = 'transform .2s ease';
    });
    shadow.getElementById('fp-zoom-out').addEventListener('click', e => {
      e.stopPropagation();
      const vw = vpOuter.clientWidth / 2;
      const vh = (vpOuter.clientHeight || 540) / 2;
      zoomAt(zoom / 1.3, vw, vh);
    ...
    shadow.getElementById('fp-zoom-fit').addEventListener('click', e => {
      e.stopPropagation();
      fitToScreen();
      vpInner.style.transition = 'transform .2s ease';
    });
    ...
    minimap.addEventListener('click', e => {
      ...
      const mx = (e.clientX - rect.left) / 160;
      const my = (e.clientY - rect.top)  / 67;
      const vw = vpOuter.clientWidth;
      const vh = vpOuter.clientHeight || 540;
      panX = vw / 2  - mx * CONTENT_W * zoom;
      panY = vh / 2  - my * CONTENT_H * zoom;
      apply(true);
    });
    ...
    if (!tt) {
      ...
      ttStyle.textContent = `
        ...
        .mhww-tt .slogo{width:100%;min-height:40px;background:#f7f9f8;border-radius:6px;margin-bottom:8px;
          display:flex;align-items:center;justify-content:center;padding:6px;overflow:hidden;}
      `;
    ...
    const tips = {
      ...
      'speaker':    {title:'\uD83C\uDF99\uFE0F Speaker Lounge', body:'A dedicated space for speakers to network, collaborate and unwind. A comfortable, curated space designed to facilitate meaningful conversations beyond the stage.'},
      'booth-45':   {type:'booth', num:'45', size:'3\xD73', sponsor:'Bupa', logoUrl:'https://static.wixstatic.com/media/958742_847c285ca4c94f01b31f2191b1df4ba5~mv2.png', body:'Bupa is an international healthcare company dedicated to helping people live longer, healthier, happier lives. In Australia, Bupa delivers quality health insurance, aged care, dental, optical and more.'},
      'booth-2':    {type:'booth', num:'2',  size:'6\xD712', sponsor:'Medibank', logoUrl:'https://static.wixstatic.com/media/958742_feff646cdb1e4fc0ae6bf2ea1f6b155b~mv2.png', body:'Medibank is one of Australia\'s leading health insurers, providing health insurance and health services to over 3.9 million customers. Medibank is committed to better health for better lives.'},
      'booth':      {type:'generic'}
    };

    function mv(e) {
      const p=14, tw=tt.offsetWidth, th=tt.offsetHeight;
    ...
    shadow.querySelectorAll('[data-tip]').forEach(el => {
      let moved = false;
      el.addEventListener('mousedown', () => { moved = false; });
      el.addEventListener('mousemove', () => { moved = true; });
      el.addEventListener('click', e => {
        if (moved) return;
        const d = tips[el.dataset.tip]; if (!d) return;
        if (d.type === 'generic')
          tt.innerHTML = `<div class="bref">Booth ${el.dataset.num} \u2014 ${el.dataset.size}</div><p>Booth available.</p>`;
        else if (d.type === 'booth')
          tt.innerHTML = `<div class="bref">Booth ${d.num} \u2014 ${d.size}</div><div class="slogo"><img src="${d.logoUrl}" alt="${d.sponsor}" style="max-width:100%;max-height:100%;object-fit:contain;"></div><p>${d.body}</p>`;
        else
          tt.innerHTML = `<h3>${d.title}</h3><p>${d.body}</p>`;
      ...
      el.addEventListener('mousemove', e => { if (!isDragging) mv(e); });
      el.addEventListener('mouseleave', () => { tt.className = 'mhww-tt'; });
    });

    // Close tooltip on outside click
    document.addEventListener('click', e => {
      if (!e.composedPath().some(n => n === vpOuter)) tt.className = 'mhww-tt';
    });
  }
}
